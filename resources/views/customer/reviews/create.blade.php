...
@section('title', 'Beri Ulasan Produk - ' . config('app.name', 'Tokobii'))
...
<div class="container-fluid px-0">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb bg-transparent p-0 mb-0" style="font-size: 0.8125rem;">
            ...
            <li class="breadcrumb-item active text-slate-800 fw-semibold" aria-current="page">Beri Ulasan</li>
        </ol>
    </nav>
        ...
        <div class="col-12 col-lg-7">
            ...
            <div class="tokobii-card">
                <div class="tokobii-card-header d-flex align-items-center justify-content-between">
                    <h5 class="fw-bold mb-0 text-slate-900" style="font-size: 1.05rem;">Beri Ulasan Produk</h5>
                ...
                <div class="p-4 p-md-5">
                    ...
                    <div class="p-3 bg-slate-50 rounded-3 border border-slate-200 mb-4 d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <span class="text-slate-400 small">Nomor Invoice: {{ $order->invoice_number }}</span>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('customer.orders.reviews.store', [$order, $item]) }}">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <select name="rating" class="form-select tokobii-select @error('rating') is-invalid @enderror" required>
                                ...
                                <option value="5" @selected(old('rating', 5) == 5)>5 Bintang (Sangat Puas)</option>
                                <option value="4" @selected(old('rating') == 4)>4 Bintang (Puas)</option>
                                <option value="3" @selected(old('rating') == 3)>3 Bintang (Cukup)</option>
                                <option value="2" @selected(old('rating') == 2)>2 Bintang (Kurang Puas)</option>
                                <option value="1" @selected(old('rating') == 1)>1 Bintang (Kecewa)</option>
                            </select>
                            @error('rating')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="comment" name="comment" rows="5" class="form-control tokobii-input @error('comment') is-invalid @enderror" placeholder="Bagikan pengalaman Anda mengenai kualitas produk, kerapihan kemasan, dsb..." required>{{ old('comment') }}</textarea>
                            @error('comment')
                                <div class="invalid-feedback d-block text-rose-600 small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
