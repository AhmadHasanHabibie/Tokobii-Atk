...
<div class="container py-4">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb bg-transparent p-0 mb-0" style="font-size: 0.8125rem;">
            <li class="breadcrumb-item"><a href="{{ route('shop') }}" class="text-decoration-none text-slate-500 hover-text-blue-600">Katalog</a></li>
            @if($product->category)
                <li class="breadcrumb-item">
                    <a href="{{ route('shop', ['category' => $product->category->slug]) }}" class="text-decoration-none text-slate-500 hover-text-blue-600">{{ $product->category->name }}</a>
                </li>
            @endif
            <li class="breadcrumb-item active text-slate-800 fw-semibold text-truncate" style="max-width: 250px;" aria-current="page">{{ $product->name }}</li>
        </ol>
    </nav>
